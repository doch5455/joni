...
async def play_logs(message, streamtype: str):
    """Yeni müzik oynatma başladığında log kanalı için kayıt oluşturur."""
...
🏷 <b>Grup:</b> <a href="{chat_link}">{message.chat.title}</a> <code>[{chat_id}]</code>  
👥 <b>Üye Sayısı:</b> <code>{uye_sayisi}</code>  
👤 <b>Kullanıcı:</b> {mention}  
🔖 <b>Kullanıcı Adı:</b> {username}  
🆔 <b>Kullanıcı ID:</b> <code>{message.from_user.id if message.from_user else '—'}</code>
...
"""
        ...
        if chat_id != LOGGER_ID:
            try:
                ...
                # Log grubunun başlığını aktif sesli sohbet sayısıyla güncelle
                try:
